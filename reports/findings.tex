...
\section*{Current ICLR 2026 extension}
This document preserves the earlier version-1 integration studies. The current application uses rubric \texttt{folio-2.0}, defaults to ICLR 2026 via OpenReview, and adds a supervised classifier of Jev outputs. A separate report, \texttt{reports/iclr2026-findings.tex} (and compiled PDF), documents 191 labeled papers split 114/38/39 for training, calibration, and testing. Test accuracy is 61.5\% versus a 53.8\% baseline; AUROC is 0.598 with a 95\% bootstrap interval of 0.405--0.778. The application exposes six probability bands and an acceptance estimate, clearly distinguished from its manuscript rubric. Retrospective PDF revisions and a selected archive limit interpretation; future-submission accuracy remains unvalidated. The current suite passes 114 backend tests and 30 browser scenarios. Historical thresholds, confidence abstention, defaults, and results below describe the earlier versions.
...
\IfFileExists{reports/measurement-table.tex}{\input{reports/measurement-table.tex}}{\input{measurement-table.tex}}
\end{table}
